...
train_itr = train_gen.flow_from_frame(data_path, trn_df, 'basefile', ['Mayo_score'], 
                                     target_size=(256, 320), batch_size=batch_size)
val_itr = test_gen.flow_from_frame(data_path, val_df, 'basefile', ['Mayo_score'], 
                                     target_size=(256, 320), batch_size=batch_size)
...
checkpointer = ModelCheckpoint(filepath=model_path+'phase2_0512_cont.h5', verbose=0, save_best_only=True, save_weights_only=True)
...
model.save_weights(model_path+'phase2_0512_cont_f.h5')

with open('output/phase2_0512_cont.pkl', 'wb') as f:
