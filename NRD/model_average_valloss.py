...
all_df = pd.read_csv(path+'cohorts30/{}/pred_comorb.csv'.format(cohort, cohort), dtype=core_dtypes_pd)
...
for trn_idx, val_idx in skf.split(train_df0, train_df0.HOSP_NRD):
    ...
    for avg_ind in range(n_avg):
        ...
        roc_auc = auc(fpr, tpr)
        model.save_weights(model_path+'testing/avg_valloss_{}{}{}_{}.h5'.format(cohort, tst_seed, val_seed, avg_ind))
    ...
    y_pred_df.to_csv(path+'cohorts30/{}/model_avg/pred_valloss_{}{}.csv'.format(cohort, tst_seed, val_seed), index=False)
    ...
    auc_df.to_csv(path+'cohorts30/{}/model_avg/auc_valloss_{}{}.csv'.format(cohort, tst_seed, val_seed), index=False)
    val_seed += 1
