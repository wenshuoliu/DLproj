...
DX_dims = np.random.randint(10, 150, n_sample)
hosp_dims = np.random.randint(1, 5, n_sample)
lrs = np.random.choice([0.001, 0.0005, 0.0002, 0.0001], n_sample)
dropouts = np.random.rand(n_sample)*0.5
batch_sizes = np.random.choice([512, 256, 128], n_sample)
penalty = np.power(10, np.random.rand(n_sample)*5-6)
...
def model_build(DX_embed_dim,hosp_embed_dim,penalty,fc_width,dropout,lr,batch_size):
    ...
    DX_embed = Embedding(input_dim=parent_mat.shape[1], output_dim=DX_embed_dim, input_length=N_DX, mask_zero=True,
...
for param in parameters:
    ...
    auccheckpoint = AUCCheckPoint(filepath=model_path+'amiccs_lstm_nomask_{}_{}_{:.4f}_{}_{:.4f}_{}_{}.h5'.format(*param), validation_y=Y_val[:, 1], validation_x=[demo_mat_val, DX1_mat_val, DX_mat_val, hosp_array_val])
    ...
    hist = model.fit([demo_mat_trn, DX1_mat_trn, DX_mat_trn, hosp_array_trn], Y_trn, 
                 batch_size=batch_size, epochs=200, callbacks=[auccheckpoint, reduce_lr, earlystop], class_weight=class_weight, 
                 validation_data=[[demo_mat_val, DX1_mat_val, DX_mat_val, hosp_array_val], Y_val], 
                verbose=2)
        
    model.load_weights(model_path+'amiccs_lstm_nomask_{}_{}_{:.4f}_{}_{:.4f}_{}_{}.h5'.format(*param))
    y = model.predict([demo_mat_tst, DX1_mat_tst, DX_mat_tst, hosp_array_tst], verbose=0)
    y_pred = y[:, 1]
    
    fpr, tpr, _ = roc_curve(y_tst, y_pred)
    roc_auc = auc(fpr, tpr)
    with open('output/hyper_tune_mask.csv', 'a') as f:
