...
parameters = list(zip(np.random.choice(depths, n_sample), np.random.choice(widths, n_sample),
                     ...
                     np.random.choice(lrs, n_sample), np.random.choice(dropouts, n_sample), np.random.choice(batch_sizes, n_sample)))
parameters = [p for p in parameters if parameters.count(p)==1]
param_tried = pd.read_csv('output/hyper_tune_large.csv')
param_tried = param_tried[['depth', 'width', 'DX_dim', 'hosp_dim', 'lr', 'dropout', 'batchsize']].values
...
def model_build(depth, width, DX_dim, hosp_dim, lr, dropout, batchsize):
...
for param in parameters:
    if not param in param_tried: 
        ...
        checkpointer = ModelCheckpoint(filepath=model_path+'ami_icd9_{}_{}_{}_{}_{}_{}_{}.h5'.format(*param), verbose=0, save_best_only=True, save_weights_only=True)
        ...
        hist = model.fit([demo_mat[:N_train, :], DX1_mat[:N_train, :], DX_mat[:N_train, :], hosp_array[:N_train]], y_train, 
                         batch_size=batch_size, epochs=150, callbacks=[checkpointer, reduce_lr, earlystop], class_weight=class_weight, 
                         validation_data=[[demo_mat[N_train:, :], DX1_mat[N_train:, :], DX_mat[N_train:, :], hosp_array[N_train:]], y_test], 
                         verbose=2)
        model.load_weights(model_path+'ami_icd9_{}_{}_{}_{}_{}_{}_{}.h5'.format(*param))
        y_pred = model.predict([demo_mat[N_train:, :], DX1_mat[N_train:, :], DX_mat[N_train:, :], hosp_array[N_train:]], batch_size=batch_size)
        fpr, tpr, _ = roc_curve(y_test[:, 0], y_pred[:, 0])
        roc_auc = auc(fpr, tpr)
        with open('output/hyper_tune_large.csv', 'a') as f:
            f.write('{0},{1},{2},{3},{4},{5},{6},{7:.3f}\n'.format(*param,roc_auc))
        del(model)
