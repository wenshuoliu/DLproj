...
for trn_idx, val_idx in skf.split(train_df0, train_df0.HOSP_NRD):
    ...
    DX1_embed0 = Embedding(input_dim=len(code_cat), output_dim=code_embed_dim, embeddings_initializer=embed_initializer, 
                          name='DX1_embed')(input_DX1)
    DX1_embed = Reshape((code_embed_dim,))(DX1_embed0)
    input_DX = Input(shape = (len(DXs),))
    DX_embed0 = Embedding(input_dim=len(code_cat), output_dim=code_embed_dim, mask_zero=False, embeddings_initializer=embed_initializer, 
    ...
    PR_embed0 = Embedding(input_dim=len(code_cat), output_dim=code_embed_dim, mask_zero=False, embeddings_initializer=embed_initializer, 
    ...
    cr =  SkipgramRegularization(n_samples, len(code_cat), lamb=lamb, name = 'skipgram')([merged1,merged2])
    ...
    auccheckpoint = AUCCheckPoint(filepath=model_path+'skipgram_temp'+str(job_index)+'.h5', 
                                  validation_y=[y_val, np.zeros(y_val.shape)], 
    ...
    hist = model.fit([DX1_array_trn, DX_mat_trn, PR_mat_trn, hosp_array_trn, other_mat_trn], [y_trn, np.zeros(y_trn.shape)], 
                     batch_size=batchsize, epochs=100, callbacks=[auccheckpoint, reduce_lr, earlystop], 
                     validation_data=[[DX1_array_val, DX_mat_val, PR_mat_val, hosp_array_val, other_mat_val], 
                                      [y_val, np.zeros(y_val.shape)]], verbose=2)
...
with open(result_file.format(job_index), 'a') as f:
    f.write('{},{},{},{},{},{:.1E},{:.1f},{},{},{},{},{},{},{},{:.3f},{},{:.5f},{:.5f},{}\n'.format(model_name, code_embed_dim, hosp_embed_dim, fc_width, md_width, lr1, dropout, batchsize, embed_file, cohort, tst_seed, n_fold, DX_rarecutpoint, PR_rarecutpoint, lamb, n_samples, auc_mean, auc_avg, y_pred_file))