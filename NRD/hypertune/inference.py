...
train_df0 = pd.read_csv(path+'cohorts30/{}/inference/index_split{}_{}.csv'.format(cohort, split_seed, train_idx))
train_df0 = train_df0.reset_index(drop=True)
tst_df = pd.read_csv(path+'cohorts30/{}/inference/index_split{}_{}.csv'.format(cohort, split_seed, 3-train_idx))
index_df = train_df0.sample(frac=0.1, random_state=24)
train_df0 = train_df0.groupby('HOSP_NRD').apply(lambda x:x.sample(frac=1., random_state=sample_seed)).reset_index(drop=True)
...
for val_ind in range(n_val):
    trn_df, val_df = train_test_split(train_df0, test_size=0.15, random_state=24+val_ind, stratify=train_df0.HOSP_NRD)
    N_trn = len(trn_df)
    ...
    for i, hosp in enumerate(hosp_cat):
        ...
        recycle_pred[i, val_ind] = y_pred.mean()
