...
parser.add_argument('--penalty_norm', type=int, default=2)
...
from keras.layers import Input, Embedding, Concatenate, LSTM, CuDNNLSTM, Lambda, Reshape, RepeatVector
...
if model_name=='setsum':
    ...
    DX_embed = Embedding(input_dim=parent_mat.shape[1], output_dim=DX_embed_dim, mask_zero=True,
                     embeddings_regularizer=Mat_reg(parent_mat, penalty, penalty_norm), name='DX_embed')(input_DX)
    DX_feature = SetSum(DX_embed_dim, activation=dense_activation)(DX_embed)
    input_demo = Input(shape=(2, ))
    input_DX1 = Input(shape=(len(DX1_cat),))
    input_hosp = Input(shape=(1,))
    hosp_embed = Embedding(input_dim=len(hosp_cat), output_dim=hosp_embed_dim, input_length=1)(input_hosp)
    hosp_embed = Reshape((hosp_embed_dim, ))(hosp_embed)
    merged = Concatenate(axis=1)([input_demo, input_DX1, DX_feature, hosp_embed])
...
elif model_name=='interact':
    input_demo = Input(shape=(2, ))
    input_DX1 = Input(shape=(len(DX1_cat),))
    input_hosp = Input(shape=(1,))
    hosp_embed = Embedding(input_dim=len(hosp_cat), output_dim=hosp_embed_dim, input_length=1)(input_hosp)
    hosp_embed = Reshape((hosp_embed_dim, ))(hosp_embed)
    merged = Concatenate(axis=1)([input_demo, input_DX1, hosp_embed])
    merged = RepeatVector(N_DX)(merged)
    input_DX = Input(shape = (N_DX,))
    DX_embed = Embedding(input_dim=parent_mat.shape[1], output_dim=DX_embed_dim, mask_zero=True,
                         embeddings_regularizer=Mat_reg(parent_mat, penalty, norm=penalty_norm), name='DX_embed')(input_DX)
    feature = Concatenate(axis=-1)([DX_embed, merged])
    feature = MaskedDense(DX_embed_dim, activation=dense_activation)(feature)
    feature = MaskedSum()(feature)
    x = Dense(fc_width, activation='relu')(feature)
    x = Dropout(dropout)(x)
    prediction = Dense(2, activation='softmax')(x)
    model = Model(inputs=[input_demo, input_DX1, input_DX, input_hosp], outputs=prediction)
